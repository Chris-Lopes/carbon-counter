...
def get_smoke_data(request):
    ...
    try:
        ...
        if arduino_serial and arduino_serial.is_open:
            ...
            raw_line = arduino_serial.readline()
            print(f"Raw bytes from Arduino: {raw_line}")  # Debug print raw bytes
            line = raw_line.decode('utf-8').strip()
            print(f"Decoded line from Arduino: '{line}'")  # Debug print decoded line
            ...
            if line.startswith("Smoke:"):
                ...
                try:
                    parts = line.split(":")
                    print(f"Split parts: {parts}")  # Debug print split parts
                    smoke_value = int(parts[1].strip())
                    print(f"Successfully parsed smoke value: {smoke_value}")  # Debug print parsed value
                except Exception as e:
                    print(f"Error parsing standard format: {e}")  # More detailed error
            elif ":" in line:
